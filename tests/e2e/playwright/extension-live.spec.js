...
// Delay helper para dar tiempo al sistema a cargar
...
test.describe('Extension MV3 Live', () => {
  test.setTimeout(120000); // Aumentado a 2 minutos
  ...
  test('carga service worker y el popup no queda calculando rutas indefinidamente', async () => {
    ...
    try {
      // Esperar a que el service worker se cargue
      ...
      // Delay para que la extension se inicialice completamente
      ...
      // Delay despues de cargar el popup
      await delay(LOAD_DELAY);
      ...
      // Esperar a que el popup termine de cargar datos
      await waitUntilPopupSettles(popup, 45000);
      await delay(LOAD_DELAY);

      await assertSettledState(popup);

      // Probar clicks en el boton de refresh
      const refreshButton = popup.locator('#refresh, [aria-label*="Actualizar"]').first();
      for (let attempt = 1; attempt <= 3; attempt++) {
        console.log(`\n[Intento ${attempt}/3] Click en refresh...`);
        await delay(LOAD_DELAY); // Delay antes del click
        await refreshButton.click();

        console.log(`[Intento ${attempt}/3] Esperando respuesta...`);
        await waitUntilPopupSettles(popup, 45000);
        await delay(LOAD_DELAY); // Delay despues de recibir respuesta

        await assertSettledState(popup);
        console.log(`[Intento ${attempt}/3] Estado verificado correctamente`);
      }
  ...
  test('botones de filtro funcionan correctamente', async () => {
    const extensionPath = path.resolve(__dirname, '../../../');
    const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pw-arbitrars-filter-'));
    ...
    try {
      ...
      // Probar botones de filtro P2P
      const filterButtons = popup.locator('.filter-btn-footer');
      const count = await filterButtons.count();
      console.log(`\nEncontrados ${count} botones de filtro`);

      for (let i = 0; i < count; i++) {
        const buttonText = await filterButtons.nth(i).textContent();
        console.log(`[Filtro ${i + 1}/${count}] Click en "${buttonText}"...`);
        await delay(LOAD_DELAY);
        await filterButtons.nth(i).click();
        await delay(LOAD_DELAY);
        console.log(`[Filtro ${i + 1}/${count}] Click exitoso`);
      }

      console.log('\n✅ Test de filtros completado');
    } finally {
      await context.close();
      try {
        fs.rmSync(userDataDir, { recursive: true, force: true });
  ...
  test('boton de configuracion abre panel', async () => {
    const extensionPath = path.resolve(__dirname, '../../../');
    const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pw-arbitrars-config-'));
    ...
    try {
      ...
      // Click en boton de configuracion
      const settingsButton = popup.locator('#settings, [aria-label*="configuración"]').first();
      console.log('\nClick en boton de configuracion...');
      await delay(LOAD_DELAY);
      await settingsButton.click();
      await delay(LOAD_DELAY);

      // Verificar que el panel de filtros avanzados se abrio
      const advancedPanel = popup.locator('#advanced-filters-panel');
      const isVisible = await advancedPanel.isVisible().catch(() => false);
      console.log(`Panel de filtros visible: ${isVisible}`);

      console.log('\n✅ Test de configuracion completado');
    } finally {
      await context.close();
      try {
        fs.rmSync(userDataDir, { recursive: true, force: true });
